...
def Pyr2Int(syn_params, sec_x, sec_id):
    ...
    if syn_params.get('initW') and syn_params.get('stdevW'):
        m = float(syn_params['initW'])
        s = float(syn_params['stdevW'])
        mean = np.log(m) - 0.5 * np.log((s/m)**2+1)
        std = np.sqrt(np.log((s/m)**2 + 1))
        weight = float(np.random.lognormal(mean,std)) # par.x(0) * rC.uniform(0.5,1.0)//rand.normal(0.5,1.5) //`rand.repick() 
        weight = min( weight, np.exp(mean+3*std))
        lsyn.initW = weight
